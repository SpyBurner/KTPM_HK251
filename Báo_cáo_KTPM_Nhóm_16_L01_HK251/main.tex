...
\newcommand{\us}[2]{%
	...
	\def\@currentlabel{#1}% ép \ref hiển thị ID thật
...
\newcommand{\nfr}[2]{%
	\refstepcounter{req}%
	\def\@currentlabel{NFR#1}% ép \ref hiển thị NFR01
	\label{nfr:#1}%
	\textbf{NRF#1}%
}
...
% ADR
\newcommand{\ADRTable}[4]{
\begin{longtable}{|p{4cm}|p{11cm}|}
	\caption{Architecture Decision Record (ADR) cho: #1}\label{tab:adr:#4} \\
	\hline
	...
	\textbf{Tiêu đề} & #1 \\ \hline
	\textbf{Status (Trạng thái)} & #2 \\ \hline
	\textbf{Context (Bối cảnh)} & #3 \\ \hline
}
